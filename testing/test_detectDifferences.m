...
folderPath = fullfile(scriptDir, '..', 'data', 'Datasets', 'Columbia Glacier');
...
for i = 1:length(output)
    ...
    % Create and warp a mask of valid pixels
    validMask = true(size(img2, 1), size(img2, 2));
    [mask, ~] = imwarp(validMask, tform, 'OutputView', imref2d(size(img1)));
    % Show overlay using imshowpair
    [diffMask, boundaries] = detectDifferences(img1, warpedImg2, 'absdiff', 0.2,mask);

    figure;
   % Left: img1 with detected change regions overlaid
    subplot(1,2,1);
    imshow(warpedImg2); hold on;
    ...
    % Right: img1 and warpedImg2 blended
    subplot(1,2,2);
    imshow(img1);
